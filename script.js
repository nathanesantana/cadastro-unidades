$(document).ready(function(){
    ...
    function preencherTable(response) {
        ...
        for (let i = 0; i < dados.length; i++){
            ...
            <tr>
                ...
                <td>${paciente.quartos}</td>
                <td>${paciente.leitos}</td>
                <td><button type="button" class="editar" data-id ="${paciente.id}" title="Editar"><i class="bi bi-pencil-square"></i></button></td>
                <td><button type="button" class="excluir" data-id="${paciente.id}" title="Excluir"><i class="bi bi-trash3"></i></button></td>
            </tr> `;
        }
    ...
    function preencherFormulario(response){
        ...
        $(`input[name="pergunta"][value="${data.internacao}"]`).prop("checked", true),
        $("#quartos").val(data.quartos),
        $("#leitos").val(data.leitos)
    }
